...
class ArucoDetectionModule(BaseModule[VideoFrame | np.ndarray]):
    ...
    def _draw_rejected(self, image: np.ndarray, rejected: list[np.ndarray]) -> np.ndarray:
        output = image.copy()
        if rejected:
            self._aruco.drawDetectedMarkers(output, rejected, None, (255, 0, 255))
        return output
    ...
    def _write_debug_images(
        ...
        if not self.debug:
            return
        self._write_debug_image("aruco_input.png", image)
    ...
    async def process(
        ...
        corners, ids, rejected = self._detect(image)
        self._write_debug_images(image, corners, ids, rejected)

        if ids is None or len(ids) == 0:
            logger.debug("No ArUco markers detected")
            return None

        marker_ids = [int(marker_id) for marker_id in ids.reshape(-1)]
        detections = [
